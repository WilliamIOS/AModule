...
Repo="AModule"
RepoList=("AModule" "AModule_test" "AModule_gray")
getRepo() {
    echo -e "\n"
    length=${#RepoList[@]}
...
TestVersionLineNumber=`grep -nE 'version.test.*=' AModule.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" AModule.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' AModule.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" AModule.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' AModule.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' AModule.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" AModule.podspec
...
pod repo push ${Repo} AModule.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
